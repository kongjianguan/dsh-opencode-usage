window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function UsageWidget() {
      ...
      // 面板钳制：任一边超出视口就平移回视口内（margin 平移，不碰 ocu-pop 动画的 transform）
      React.useLayoutEffect(function () {
        ...
        var dx = Math.min(0, vw - 8 - r.right)
        dx = Math.max(dx, 8 - r.left)
        var dy = Math.min(0, vh - 8 - r.bottom)
        dy = Math.max(dy, 8 - r.top)
