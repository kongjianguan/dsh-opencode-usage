    ...
    function UsageWidget() {
      ...
      React.useLayoutEffect(function () {
        ...
        var clamp = function () {
          ...
          // 水平：先贴按钮右缘（且不超视口右缘）
          var left = Math.min(rr.right - w, vw - 8 - w)
          var avail = contentLeft()
          if (left < avail) {
            // 贴按钮会盖左侧栏：右移到可用区左缘，前提是放得下（不超视口右缘）
            var L2 = Math.min(avail, vw - 8 - w)
            if (L2 >= avail - 1) left = L2
            // 放不下（聊天区 < 弹窗宽）→ 保留贴按钮位置，允许覆盖左侧栏（避无可避）
          }
