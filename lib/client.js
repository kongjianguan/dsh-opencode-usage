window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var CSS = [
      ...
      '.ocu-circle-panel{z-index:100;box-sizing:border-box;border:1px solid var(--dsw-alias-border-inverted);background:var(--dsw-specific-menu);width:264px;max-width:calc(100vw - 16px);box-shadow:var(--dsw-shadow-lv3);color:var(--dsw-alias-label-secondary);cursor:default;border-radius:12px;padding:12px;font-size:12px;line-height:20px;position:absolute;bottom:calc(100% + 8px);right:0;animation:ocu-pop .12s ease-out}',
    ...
    function UsageWidget() {
      ...
      // 面板钳制：margin 是相对包含块（按钮）的偏移，天然跟随输入框移动。
      // 方向：right:0 定位下 margin-left 正 → 左移、负 → 右移；
      //      bottom 定位下 margin-top 正 → 上移、负 → 下移。
      // 打开期间每帧（rAF）检查，位置达标则不再写入（幂等，无震荡），
      // 布局在打开后再变化（侧边栏开关/拖动等）也会被下一帧纠正。
      React.useLayoutEffect(function () {
        ...
        var clamp = function () {
          ...
          var w = el.offsetWidth || 264
          var vw = window.innerWidth
          var vh = window.innerHeight
          var left = Math.max(8, r.right - w) // 目标视口左缘：优先贴输入框右缘
          if (left + w > vw - 8) left = Math.max(8, vw - 8 - w) // 右缘也超 → 贴视口右缘
          if (Math.abs(r.left - left) > 1) el.style.marginLeft = (r.left - left) + 'px'
          var top = Math.max(8, r.top) // 目标视口顶缘（bottom 定位向上展开，顶部不足则下移）
          if (Math.abs(r.top - top) > 1) el.style.marginTop = (r.top - top) + 'px'
