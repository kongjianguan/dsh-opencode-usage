window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function UsageWidget() {
      ...
      // 面板钳制：直接以 left/right/top/bottom 重新定位（不用 margin——
      // 对 right/bottom 定位的元素，margin 的方向语义相反，会把面板推错方向）
      React.useLayoutEffect(function () {
        ...
        var clamp = function () {
          ...
          // 水平：优先右缘贴输入框右缘；左缘不足 8px 则整体右移
          var left = Math.max(8, r.right - w)
          if (left + w > vw - 8) left = Math.max(8, vw - 8 - w) // 右缘也超 → 贴视口右缘
          el.style.left = left + 'px'
          el.style.right = 'auto'
          // 垂直：保持当前底缘（bottom 定位向上展开）；顶部不足 8px 则改贴顶
          if (r.top < 8) {
            el.style.top = '8px'
            el.style.bottom = 'auto'
          } else {
            el.style.top = 'auto'
            el.style.bottom = Math.max(8, vh - r.bottom) + 'px'
          }
        }
        clamp()
        window.addEventListener('resize', clamp)
        return function () { window.removeEventListener('resize', clamp) }
      }, [open])
