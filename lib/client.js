window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var CSS = [
      ...
      '.ocu-panel{position:absolute;top:calc(100% + 6px);right:0;z-index:9999;min-width:290px;max-width:calc(100vw - 16px);background:var(--dsw-alias-bg-overlay);border:1px solid color-mix(in srgb,var(--dsw-alias-border-l2) 55%,transparent);border-radius:12px;box-shadow:inset 0 1px 0 rgba(255,255,255,.16),0 12px 32px rgba(0,0,0,.35);padding:10px 12px;font-size:12px;color:var(--dsw-alias-label-primary);animation:ocu-pop .14s ease-out}',
      '.ocu-panel-left{right:auto;left:0}',
      '.ocu-panel-up{top:auto;bottom:calc(100% + 6px)}',
    ...
    function UsageChip(props) {
      ...
      var flipState = React.useState('')
      var flip = flipState[0]
      var setFlip = flipState[1]
      ...
      React.useLayoutEffect(function () {
        ...
        var cls = ''
        if (r.right > vw - 8) cls += ' ocu-panel-left'
        if (r.bottom > vh - 8) cls += ' ocu-panel-up'
        setFlip(cls)
      }, [expanded])
