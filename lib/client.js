window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function UsageChip(props) {
      ...
      // 面板钳制：测量实际位置，左右/上下任一边超出视口就平移回视口内
      // （用 margin 平移，不碰 ocu-pop 动画使用的 transform）
      React.useLayoutEffect(function () {
        ...
        var dx = Math.min(0, vw - 8 - r.right)  // 右缘超 → 向左移
        dx = Math.max(dx, 8 - r.left)           // 左缘超 → 向右移
        var dy = Math.min(0, vh - 8 - r.bottom) // 下缘超 → 向上移
